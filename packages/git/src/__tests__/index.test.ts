...
describe('@monoweave/git', () => {
  ...
  describe('gitTag', () => {
    it('fails if invariant not respected', async () => {
      ...
      await expect(async () => gitTag('pkg@1.0.0', { cwd, context })).rejects.toMatchInlineSnapshot(
  ...
  describe('gitGlob', () => {
    it('correctly lists globbed files', async () => {
      ...
      expect(await gitGlob(['other.txt', '**/test*.txt'], { cwd, context })).toEqual(
        expect.arrayContaining(['other.txt', 'test.txt', 'child/test1.txt', 'child/test2.txt']),
      )
    })
  })
})
