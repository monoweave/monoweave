  ...
  async parseConfiguration(): Promise<{
    ...
    const config: RecursivePartial<MonoweaveConfiguration> = {
      ...
      changesetIgnorePatterns: this.noChangesetIgnorePatterns
        ? []
        : (this.changesetIgnorePatterns ?? configFromFile?.changesetIgnorePatterns ?? undefined),
      ...
      prereleaseNPMTag: this.prereleaseNPMTag ?? configFromFile?.prereleaseNPMTag ?? undefined,
      packageGroupManifestField:
        this.packageGroupManifestField ?? configFromFile?.packageGroupManifestField ?? undefined,
      packageGroups: configFromFile?.packageGroups,
      versionStrategy: {
        minimumStrategy:
          this.minimumVersionStrategy === 'none'
            ? undefined
            : (this.minimumVersionStrategy ?? configFromFile?.versionStrategy?.minimumStrategy),
        versionFolder:
          this.versionFolder ?? configFromFile?.versionStrategy?.versionFolder ?? undefined,
      },
    }
