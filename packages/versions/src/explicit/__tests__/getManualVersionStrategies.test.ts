...
describe('Version File Parsing', () => {
  it('throws an error on invalid files', async () => {
    await expect(() =>
      parseDeferredVersionFile(['"@scope/package": minor', '---', 'Changelog entry.'].join('\n')),
...
describe('getManualVersionStrategies', () => {
  ...
  it('ignores failures obtaining upstream branch', async () => {
    ...
    vi.spyOn(mockGit, 'gitUpstreamBranch').mockRejectedValueOnce(() => {
  ...
  it('merges version strategies across version files', async () => {
    ...
    const versionFolder = npath.fromPortablePath(ppath.resolve(context.project.cwd, '.monoweave'))
  ...
  it('skips version files that have already been consumed on the remote tracking branch', async () => {
    ...
    vi.spyOn(mockGit, 'gitUpstreamBranch').mockResolvedValue('some-upstream')
    vi.spyOn(mockGit, 'gitDiffTree').mockResolvedValue([versionFilePath1].join('\n'))
