...
import { type ChangesetSchema, type YarnContext } from '@monoweave/types'
...
describe('prependChangelogFile', () => {
  ...
  it('writes changelogs for each package if token present', async () => {
    ...
    expect(onDiskChangelogPkg1).not.toEqual(expect.stringContaining(changeset['pkg-2'].changelog!))
    ...
    expect(onDiskChangelogPkg2).not.toEqual(expect.stringContaining(changeset['pkg-1'].changelog!))
  })
