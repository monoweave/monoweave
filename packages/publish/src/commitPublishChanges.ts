...
export const createPublishCommit = async ({
    ...
    if (config.git.tag && gitTags?.size && !config.persistVersions) {
        ...
        // that we're running monoweave against since this marks the commit as the
        // "last one published". This informs later monoweave runs so that they don't republish
        // the changes.
