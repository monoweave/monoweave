...
describe('Monoweave', () => {
  ...
  it('logs an error if publishing fails', async () => {
    const spyPublish = vi
      ...
      .mockRejectedValue(
        new Error('Artificially induced error in a test! This is meant to fail! Ignore this.'),
      )
  ...
  it('does not push tags if push disabled', async () => {
    ...
    const result = await monoweave({
      ...monoweaveConfig,
      git: { ...monoweaveConfig.git, push: false },
    })
  ...
  it('does not create tags if git tag mode disabled', async () => {
    ...
    const result = await monoweave({
      ...monoweaveConfig,
      git: { ...monoweaveConfig.git, push: true, tag: false },
    })
  ...
  it('does not restore package.jsons if persist versions is true', async () => {
    const config = { ...monoweaveConfig, persistVersions: true }
    ...
    try {
      ...
      const result = await monoweave(config)
  ...
  it('autocommits changelogs if changelogFilename is set', async () => {
    ...
    const autoCommit = mockGit._getRegistry_().commits[mockGit._getRegistry_().commits.length - 1]
    ...
    expect(mockGit._getRegistry_().pushedCommits).toEqual(expect.arrayContaining([autoCommit.sha]))
  })
  ...
  it('autocommits changelogs and package.json files if both options are true', async () => {
    ...
    const autoCommit = mockGit._getRegistry_().commits[mockGit._getRegistry_().commits.length - 1]
    const autoCommitFiles = mockGit._getRegistry_().filesModified.get(autoCommit.sha)
    expect(autoCommitFiles).toEqual(expect.arrayContaining([changelogFilename, '**/package.json']))
  ...
  it('does not autocommit if changelogFilename and persistVersions are unset', async () => {
    ...
    const preMonoweaveLatestCommit =
    ...
    const postMonoweaveLatestCommit =
      mockGit._getRegistry_().commits[mockGit._getRegistry_().commits.length - 1]
    expect(preMonoweaveLatestCommit).toEqual(postMonoweaveLatestCommit)
  })
