...
import { prependChangelogFile } from '@monoweave/changelog'
import {
  backupPackageJsons,
  clearBackupCache,
  patchPackageJsons,
...
import logging, { ErrorsReported, InvariantError } from '@monoweave/logging'
import {
  createPublishCommit,
  determineGitTags,
...
const monoweave = async (
  ...
  if (config.plugins?.length) {
    const requireFromCwd = createRequire(npath.join(npath.fromPortablePath(cwd), 'package.json'))

    for (const pluginEntry of config.plugins) {
      const [plugin, pluginOptions] = typeof pluginEntry === 'string' ? [pluginEntry] : pluginEntry
  ...
  const pipeline = async (report: StreamReport): Promise<void> => {
    ...
    await report.startTimerPromise('Determine New Versions', { skipIfEmpty: false }, async () => {
    ...
    if (config.git.tag) {
      await report.startTimerPromise('Determine Git Tags', { skipIfEmpty: false }, async () => {
    ...
    try {
      ...
      await report.startTimerPromise('Committing Changes', { skipIfEmpty: true }, async () => {
        if (!workspaces.size) return
      ...
      if (!config.dryRun) {
        ...
        await report.startTimerPromise('Validating Git Config', { skipIfEmpty: true }, async () => {
          if (!workspaces.size) return
      ...
      await report.startTimerPromise('Publishing Packages', { skipIfEmpty: false }, async () => {
      ...
      await report.startTimerPromise('Pushing Commit', { skipIfEmpty: true }, async () => {
        ...
        if (config.dryRun) {
          ...
          // Once https://github.com/monoweave/monoweave/issues/493 is merged, we'll have a path forward for
      ...
      await report.startTimerPromise(
        ...
        { skipIfEmpty: true },
        async () => void (await hooks.onReleaseAvailable.promise(context, config, changeset)),
      )

      logging.info('Monoweave completed successfully', { report })
    } finally {
