...
By default, created releases are marked as the latest release on GitHub. To change this behaviour, set `makeLatest` to `false`:

```raw
plugins: [
  ['@monoweave/plugin-github', { makeLatest: false }]
]
```
