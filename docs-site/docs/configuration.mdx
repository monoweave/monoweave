...
### monoweave/preset-manual

Similar to `@monoweave/preset-recommended` except that parsing of commit messages for determining the next version is disabled. Instead, monoweave exclusively determines the version from `.monoweave` version files.

#### monoweave/preset-no-artifacts (formerly `preset-legacy`)
...
See the [Configuration API reference](/api/interfaces/MonoweaveConfiguration).
